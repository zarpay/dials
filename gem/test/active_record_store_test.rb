...
class ActiveRecordStoreTest < Minitest::Test
  ...
  def self.establish_schema!
    ...
    ActiveRecord::Schema.define do
      ...
      add_index :dials, %i[key scope seq], unique: true
      add_index :dials, :key
    end
    @schema_ready = true
  end
  ...
  def entries = Dials::ActiveRecord::Entry
  ...
  def test_a_global_is_the_stream_at_the_empty_scope
    Dials.set(:merchant_fee_bps, 150, actor: ACTOR)
    row = entries.sole
    assert_equal Dials::Scope::GLOBAL, row.scope
    assert_equal "set", row.action
    assert_equal 1, row.seq
    assert_equal "150", row.value
  end

  def test_every_write_appends_and_nothing_is_ever_mutated_or_deleted
    Dials.set(:merchant_fee_bps, 150, actor: ACTOR)
    Dials.set(:merchant_fee_bps, 90, scope: { market: "KE" }, actor: ACTOR)
    Dials.clear(:merchant_fee_bps, actor: ACTOR)

    assert_equal 3, entries.count, "state, history, and clock are the same appended rows"
    global = entries.where(scope: Dials::Scope::GLOBAL).order(:seq)
    assert_equal %w[set clear], global.map(&:action)
    assert_equal [1, 2], global.map(&:seq)

    # The clear ended the global's live override; the scoped stream is
    # untouched and the history of both is fully preserved.
    assert_equal 90, Dials.get(:merchant_fee_bps, market: "KE")
    assert_equal 100, Dials.get(:merchant_fee_bps, market: "NG"), "global is back to the code default"
  end

  def test_a_cleared_override_reads_as_absent_but_its_history_remains
    Dials.set(:merchant_fee_bps, 90, scope: { market: "KE" }, actor: ACTOR)
    Dials.clear(:merchant_fee_bps, scope: { market: "KE" }, actor: ACTOR)

    assert_equal 100, Dials.get(:merchant_fee_bps, market: "KE")
    assert_equal({}, Dials.scoped_overrides(:merchant_fee_bps))
    assert_equal 2, entries.count, "clearing appends; it never deletes"
  end

  def test_noop_clear_appends_nothing
    assert_equal false, Dials.clear(:merchant_fee_bps, actor: ACTOR)
    assert_equal 0, entries.count
  end
  ...
  def test_false_survives_the_database_round_trip
    ...
    assert_equal "false", entries.sole.value, "false is stored as JSON text, never SQL NULL"
  end

  def test_scope_is_stored_canonically
    Dials.set(:free_delivery_threshold, 25, scope: { platform: :ios, "market" => "KE" }, actor: ACTOR)
    assert_equal '{"market":"KE","platform":"ios"}', entries.sole.scope
  end

  def test_changes_derive_old_values_from_the_previous_row
    Dials.set(:merchant_fee_bps, 150, actor: ACTOR)
    ...
    assert changes.first.global?, "history reports nil scope for global changes"
  end
  ...
  # The gap-commit scenario: transaction A claims row id 5 but commits AFTER
  # transaction B commits id 10. A process that cached version at max-id 10
  # would never notice A's late commit if max id were the whole version —
  # the row count is what catches it.
  def test_version_catches_a_late_commit_with_a_lower_row_id
    Dials.configure { |c| c.cache_ttl = 0 }

    entries.insert_all!([{ id: 10, key: "merchant_fee_bps", scope: Dials::Scope::GLOBAL,
                           seq: 1, action: "set", value: "150", created_at: Time.now.utc }])
    assert_equal 150, Dials.get(:merchant_fee_bps, market: "KE") # cache warm at version [1, 10]

    # The "late commit": a lower row id appears without max(id) moving. Its
    # higher seq makes it the stream's newest row.
    entries.insert_all!([{ id: 5, key: "merchant_fee_bps", scope: Dials::Scope::GLOBAL,
                           seq: 2, action: "set", value: "175", created_at: Time.now.utc }])

    assert_equal 175, Dials.get(:merchant_fee_bps, market: "KE"),
                 "count moved 1 → 2, so the probe must rebuild even though max(id) is still 10"
  end

  def test_corrupt_row_is_quarantined_not_fatal
    Dials.set(:merchant_fee_bps, 150, actor: ACTOR)
    entries.insert_all!([{ key: "support_email", scope: Dials::Scope::GLOBAL, seq: 1,
                           action: "set", value: "{definitely not json", created_at: Time.now.utc }])
    Dials.reload!
  ...
  def test_malformed_scope_is_quarantined_not_fatal
    Dials.set(:merchant_fee_bps, 90, scope: { market: "KE" }, actor: ACTOR)
    entries.insert_all!([{ key: "merchant_fee_bps", scope: "not-json", seq: 1,
                           action: "set", value: "77", created_at: Time.now.utc }])
    Dials.reload!
  ...
  def test_uncommitted_writes_never_poison_the_shared_cache
    ...
    Dials::ActiveRecord::Entry.transaction do
  ...
  def test_corrupt_rows_are_quarantined_from_history
    Dials.set(:merchant_fee_bps, 150, actor: ACTOR)
    entries.insert_all!([
                          # Valid JSON, invalid shape: a scope must be an object.
                          { key: "merchant_fee_bps", scope: "42", seq: 1, action: "set",
                            value: "1", created_at: Time.now.utc },
                          { key: "merchant_fee_bps", scope: '{"market":"KE"}', seq: 1, action: "set",
                            value: "{broken", created_at: Time.now.utc }
                        ])
  ...
  def test_writes_bypassing_the_gem_need_a_manual_reload
    ...
    # counter, so the staleness probe cannot see them — each process needs a
    # manual Dials.reload!. This test pins that documented limitation.
    entries.create!(key: "support_email", scope: Dials::Scope::GLOBAL, seq: 1,
                    action: "set", value: JSON.generate("ops@example.com"))
    Dials.reload!
  ...
  def test_cas_write_against_an_absent_override_and_token_chaining
    ...
    # The returned token is the stream's new live seq — usable for the next write.
  ...
  def test_tokens_are_stream_seqs_so_recreation_never_reuses_one
    ...
    first = Dials.store.override_version(:merchant_fee_bps, Dials::Scope::GLOBAL)
    Dials.clear_merchant_fee_bps(actor: ACTOR)
    Dials.adjust_merchant_fee_bps(200, actor: ACTOR)

    assert_operator Dials.store.override_version(:merchant_fee_bps, Dials::Scope::GLOBAL), :>, first,
                    "a cleared-and-recreated override continues its stream — no seq is ever revisited"
  end

  def test_stale_write_rolls_back_with_nothing_appended
    state = Dials.overview.dials.find { |d| d.key == :merchant_fee_bps }
    Dials.adjust_merchant_fee_bps(150, actor: ACTOR, market: "BD") # appears after the page rendered

    rows_before = entries.count
    assert_raises(Dials::StaleWrite) do
      Dials.adjust_merchant_fee_bps(999, actor: ACTOR, market: "BD",
                                    expected_version: state.scoped_override_versions[{ market: "BD" }] ||
                                                      Dials::ABSENT_VERSION)
    end

    assert_equal rows_before, entries.count, "a refused write appends nothing"
    assert_equal 150, Dials.merchant_fee_bps(market: "BD"), "the stale write left the value untouched"
  end
  ...
  def test_stale_write_is_not_a_retryable_error
    ...
    # state and succeed, silently defeating the mechanism. Pin the class
  ...
  def test_a_lost_seq_claim_is_caught_by_the_unique_index
    Dials.adjust_merchant_fee_bps(150, actor: ACTOR)

    # The atomicity mechanism itself: claiming an already-taken seq slot must
    # be rejected by the database, not silently accepted.
    entries.insert_all!([{ key: "merchant_fee_bps", scope: Dials::Scope::GLOBAL, seq: 2,
                           action: "set", value: "175", created_at: Time.now.utc }])
    assert_raises(ActiveRecord::RecordNotUnique) do
      entries.insert_all!([{ key: "merchant_fee_bps", scope: Dials::Scope::GLOBAL, seq: 2,
                             action: "set", value: "999", created_at: Time.now.utc }])
    end
  end
end
