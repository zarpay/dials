...
class ScopeTest < Minitest::Test
  ...
  def multi
    @multi ||= Dials::Definition.new(:d, default: 1, type: :integer,
                                          dimensions: { market: %w[KE NG], platform: %w[ios web] })
  end
  ...
  def test_scope_on_dimensionless_dial_rejected
  ...
  def test_open_dimension_accepts_any_nonempty_value
    open = Dials::Definition.new(:o, default: 1, type: :integer, dimensions: [:tenant])
  ...
  def test_canonical_scope_is_byte_capped_for_the_indexed_column
    two_dims = Dials::Definition.new(:t, default: 1, type: :integer, dimensions: %i[tenant region])
