...
class GeneratedMethodsTest < Minitest::Test
  ...
  def test_declaring_a_dial_generates_its_three_methods
    ...
    %i[merchant_fee_bps adjust_merchant_fee_bps clear_merchant_fee_bps].each do |name|
      assert_respond_to Dials, name
    end
  end

  def test_reader_resolves_through_the_same_layers_as_get
    define_standard_dials
    assert_equal 100, Dials.merchant_fee_bps(market: "KE")
    ...
    assert_equal 250, Dials.merchant_fee_bps(market: "NG")
  end

  def test_reader_enforces_exact_scope
    define_standard_dials
    assert_raises(Dials::InvalidScope) { Dials.merchant_fee_bps }
    assert_raises(Dials::InvalidScope) { Dials.signups_enabled(market: "KE") }
  end
  ...
  def test_testing_overrides_are_visible_through_the_reader
    define_standard_dials
    Dials::Testing.with_overrides(merchant_fee_bps: 999) do
      assert_equal 999, Dials.merchant_fee_bps(market: "KE")
    end
    assert_equal 100, Dials.merchant_fee_bps(market: "KE")
  end
  ...
  def test_registry_reset_removes_generated_methods
    ...
    refute_respond_to Dials, :merchant_fee_bps
    assert_raises(NoMethodError) { Dials.merchant_fee_bps(market: "KE") }
  end
  ...
  def test_name_collision_with_an_existing_method_raises_and_registers_nothing
    Dials.define_singleton_method(:taken) { :occupied }

    error = assert_raises(Dials::InvalidDefinition) do
      Dials.define { dial :taken, default: 1, type: :integer }
    end
    assert_match(/taken/, error.message)
    refute Dials.registry.defined?(:taken)
    refute_respond_to Dials, :adjust_taken
  ensure
    Dials.singleton_class.remove_method(:taken)
  end

  def test_a_dial_cannot_shadow_the_facade_api
    error = assert_raises(Dials::InvalidDefinition) do
      Dials.define { dial :store, default: 1, type: :integer }
    end
    assert_match(/Dials.store, which already exists/, error.message)
    refute Dials.registry.defined?(:store)
  end
