...
RSpec.describe "Dial resolution", type: :model do
  ...
  it "layers global override and variation, and clears back down layer by layer" do
    ...
    expect(Dials.use_checkout_fee_bps(market: "BD")).to eq(300) # back to global
  ...
  it "keeps a variation alive when the global override is cleared" do
    ...
    expect(Dials.use_checkout_fee_bps(market: "BD")).to eq(120)
    expect(Dials.use_checkout_fee_bps(market: "KE")).to eq(250)
  end
  ...
  it "stores and serves false (the kill-switch requirement)" do
    ...
    Dials.reload!
    expect(Dials.use_signups_enabled).to be(false)
  end
  ...
  it "resolves multi-dimension scopes exactly" do
    ...
    expect(Dials.use_free_delivery_threshold(market: "KE", platform: "ios")).to eq(2_500)
    expect(Dials.use_free_delivery_threshold(market: "KE", platform: "web")).to eq(5_000)
    expect(Dials.use_free_delivery_threshold(market: "NG", platform: "ios")).to eq(5_000)
  end
  ...
  it "round-trips json dials (string keys, like any JSON API)" do
    ...
    Dials.reload!
    expect(Dials.use_welcome_banner(locale: "sw")).to eq({ "headline" => "Karibu!", "cta" => "Anza" })
    expect(Dials.use_welcome_banner(locale: "en")["headline"]).to eq("Welcome to Bazario")
  end

  it "rejects reads with wrong scopes" do
    expect { Dials.use_checkout_fee_bps }.to raise_error(Dials::InvalidScope)
    expect { Dials.use_checkout_fee_bps(market: "US") }.to raise_error(Dials::InvalidScope)
    expect { Dials.use_signups_enabled(market: "KE") }.to raise_error(Dials::InvalidScope)
    expect { Dials.use_free_delivery_threshold(market: "KE") }.to raise_error(Dials::InvalidScope)
  end
