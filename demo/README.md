...
three platforms (ios, android, web), with a Courier subsystem that owns its
own dials. Every file is annotated to teach one part of the gem:
...
| `config/initializers/dials_courier.rb` | A namespace: the Courier subsystem's own registry, table and change log |
...
| `app/services/courier/parcel_policy.rb` | Consuming a namespace's dials from inside its own subsystem |
...
| `spec/dials/namespace_spec.rb` | Two namespaces, two tables: separate keys, rows, change logs and test pins |
| `spec/services/**` | Consuming and testing dials in domain code (`Dials::Testing.with_overrides`) |
| `spec/requests/admin/dials_spec.rb` | The full HTTP write contract, including `false` over the wire and one surface per namespace |
